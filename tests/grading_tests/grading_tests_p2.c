 ...
 */
Test(sf_memsuite_grading, malloc_free_lifo, .init = sf_mem_init, .fini = sf_mem_fini)
{
    size_t sz = 200;
    void * x = sf_malloc(sz);
...
Test(sf_memsuite_grading, realloc_smaller, .init = sf_mem_init, .fini = sf_mem_fini)
{
    ...
    size_t exp_sz = PAGE_SZ - ADJUSTED_BLOCK_SIZE(nsz) - sizeof(sf_prologue) - sizeof(sf_epilogue);
    _assert_free_block_count(exp_sz, 1);
    _assert_errno_eq(0);
}
...
Test(sf_memsuite_grading, realloc_same, .init = sf_mem_init, .fini = sf_mem_fini)
{
    ...
    size_t exp_sz = PAGE_SZ - ADJUSTED_BLOCK_SIZE(sz) - sizeof(sf_prologue) - sizeof(sf_epilogue);
    _assert_free_block_count(exp_sz, 1);
...
Test(sf_memsuite_grading, free_footer_mismatch, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT, .timeout = 5)
{
    //not passing 
    size_t sz = 1;
    void * x = sf_malloc(sz);

    // this will cause footer mismatch
    NEXT_BLOCK(PAYLOAD_TO_BLOCK(x))->prev_footer = PAYLOAD_TO_BLOCK(x)->header;

    sf_free(x);
    cr_assert_fail("SIGABRT should have been received");
}

Test(sf_memsuite_grading, free_block_too_small, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT, .timeout = 5)
{
    size_t sz = 1;
    void * x = sf_malloc(sz);
