...
void _assert_heap_is_valid(void)
{
    ...
    heap_p += (sizeof(sf_prologue) - sizeof(sf_footer));
    char * stop = end_heap - sizeof(sf_epilogue) - sizeof(sf_footer);
    sf_block * bp;
    int limit = LOOP_LIMIT;
...
{
    //printf("alloc: %d\n", alloc);
    //printf("pointer to payload: %ld\n", bp);
    //printf("alloc macro: %d\n", ALLOCATED(bp));
